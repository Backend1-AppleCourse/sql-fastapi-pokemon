...
import requests
import pymysql
from app.db.pokemon_queries import *
from app.db.trainer_queries import *
from app.db.trade_queries import *
...
class PokemonDB:
    ...
    def evolve_pokemon(self, trainer_name: str, pokemon_name: str):
        with self.connection.cursor() as cursor:
            ...
            types = [t['type']['name'] for t in evolved_pokemon_data['types']]
            for type_name in types:
                cursor.execute(add_type_query(), (type_name,))
                self.connection.commit()
                cursor.execute(get_type_id_query(), (type_name,))
                type_id = cursor.fetchone()['ID']
                cursor.execute(add_pokemon_type_query(), (evolved_pokemon_id, type_id))
            self.connection.commit()
...
def get_db_connection():
    connection_params = {
        ...
        'password': os.getenv('MYSQL_PASSWORD', 'html4826'),
        'database': os.getenv('MYSQL_DATABASE', 'pokemon_data'),
